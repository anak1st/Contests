...
auto typoke(Symbol<X...>, Symbol<A>, Symbol<Y>...)
    -> decltype(typoke(Symbol<X..., A>(), Symbol<Y>()...));
...
#define TYPESTRING16(n, x)                                                           \
    impl::tygrab<0x##n##0>(x), impl::tygrab<0x##n##1>(x), impl::tygrab<0x##n##2>(x), \
    impl::tygrab<0x##n##3>(x), impl::tygrab<0x##n##4>(x), impl::tygrab<0x##n##5>(x), \
    impl::tygrab<0x##n##6>(x), impl::tygrab<0x##n##7>(x), impl::tygrab<0x##n##8>(x), \
    impl::tygrab<0x##n##9>(x), impl::tygrab<0x##n##A>(x), impl::tygrab<0x##n##B>(x), \
    impl::tygrab<0x##n##C>(x), impl::tygrab<0x##n##D>(x), impl::tygrab<0x##n##E>(x), \
    impl::tygrab<0x##n##F>(x)
...
template <typename a, typename b>
struct add
: public impl::add<a::type::numer, a::type::denom, b::type::numer, b::type::denom> {};

template <typename a, typename b>
struct sub
: public impl::sub<a::type::numer, a::type::denom, b::type::numer, b::type::denom> {};

template <typename a, typename b>
struct mul
: public number<a::type::numer * b::type::numer, a::type::denom * b::type::denom> {};

template <typename a, typename b>
struct div_
: public number<a::type::numer * b::type::denom, a::type::denom * b::type::numer> {};

template <typename a, typename b>
struct is_greater : public impl::is_greater<a::type::numer, a::type::denom,
                                            b::type::numer, b::type::denom> {};

template <typename a, typename b>
struct is_less
: public impl::is_less<a::type::numer, a::type::denom, b::type::numer, b::type::denom> {};
...
template <typename a, typename b>
struct is_equal_impl<a, b, number_tag, number_tag>
: public boolean<(a::type::numer == b::type::numer && a::type::denom == b::type::denom)> {
};

template <typename a, typename b>
struct is_equal_impl<a, b, symbol_tag, symbol_tag>
: public boolean<(a::type::value == b::type::value)> {};
...
struct length
: public if_else<is_null<items>, number<0>, add<number<1>, length<cdr<items>>>> {};

template <typename items, typename n>
struct list_ref : public if_else<is_equal<n, number<0>>, car<items>,
                                 list_ref<cdr<items>, sub<n, number<1>>>> {};
...
struct map2 {
    using type = typename if_else<
        or_<is_null<seq0>, is_null<seq1>>, null,
...
struct accumulate
: public if_else<is_null<seq>, initial, op<car<seq>, accumulate<op, initial, cdr<seq>>>> {
};
...
struct reverse {
    ...
    struct iter : public if_else<is_null<items2>, result,
                                 iter<cdr<items2>, cons<car<items2>, result>>> {};
...
struct filter {
    using type =
        typename cond<is_null<seq>, null, op<car<seq>>,
                      cons<car<seq>, filter<op, cdr<seq>>>, filter<op, cdr<seq>>>::type;
    using tag = typename type::tag;
};
